...
### 1. **yaml-cpp**
```bash
cd rinbo_sbRIO_ws/third_party/yaml-cpp/
mkdir build && cd build
cmake .. -DCMAKE_PREFIX_PATH=$HOME/rinbo_sbRIO_ws/install -DCMAKE_INSTALL_PREFIX=$HOME/rinbo_sbRIO_ws/install
make -j16
sudo make install
```
...
### 2. **Eigen**
```bash
cd rinbo_sbRIO_ws/third_party/eigen/
mkdir build && cd build
cmake .. -DCMAKE_PREFIX_PATH=$HOME/rinbo_sbRIO_ws/install -DCMAKE_INSTALL_PREFIX=$HOME/rinbo_sbRIO_ws/install
make -j16
sudo make install
...
```bash
cd rinbo/rinbo_fpga_driver/
mkdir build && cd build
cmake .. -DCMAKE_PREFIX_PATH=$HOME/rinbo_sbRIO_ws/install -DCMAKE_INSTALL_PREFIX=$HOME/rinbo_sbRIO_ws/install -DOPENSSL_ROOT_DIR=$HOME/rinbo_sbRIO_ws/install/ssl
make -j16
```
